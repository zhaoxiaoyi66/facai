...
| 交易复盘 / 投资笔记 | `ui/discipline_review.py` | 投资金句库、学习笔记和交易错题本 | 添加记录在顶部，复杂统计弱化 | 保留一级入口 |
| 交易日志 | `ui/trade_journal.py` | 记录交易行为 | 表单和历史记录分离 | 保留一级入口 |
| 个股研究 | `ui/stock_detail.py` | 单股深度研究、回撤规律、指标口径 | 默认加载当前股票，刷新按钮触发更新 | 保留；继续避免全池批量计算 |
| 研报中心 | `ui/ai_stock_radar.py` | 单票研报、价格区间和买区辅助判断 | 默认读缓存，单股详情再算深层信息 | 保留一级入口；内部 route key 兼容旧名 |
| 数据复核 | `app.py`, `ui/manual_review.py` | 后验验证和数据质量复核 | 作为分组入口 | 当前只保留信号表现等后验验证入口 |
...
| 新闻雷达 | `ui/news_radar.py`, `data/news_radar.py` | 持仓/观察池新闻事件雷达 | 默认读新闻缓存，刷新和翻译由按钮触发 | 保留一级入口 |
