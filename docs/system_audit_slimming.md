...
| 交易错题本 | `discipline-review` | 交易错题本 | 高频使用 | 旧纪律复盘命名已基本收口 | 保留一级入口 |
| 交易日志 | `trade-journal` | 交易日志 | 高频使用 | 与错题本边界清晰：日志记录操作，错题本记录错误 | 保留 |
| 个股研究 | `detail` | 个股研究 | 高频使用 | 与价格位置页共享买区/评分信息，但深度不同 | 保留 |
| 价格位置 | `ai-radar` / 内部仍兼容 `AI Stock Radar` | 价格位置 | 高频使用 | 用户界面仍有少量旧 Radar 文案，已修复显眼入口 | 保留，内部 key 暂不改 |
| 数据复核 | `manual-review` | 数据复核 | 中低频使用 | 数据补齐与复核入口，功能复杂 | 保留，维持折叠/队列化 |
| 周末价差 | `weekend-spread` | 周末价差 | 中频专项工具 | 历史包袱最多，有重复回放函数和映射审计旧入口 | 保留在数据复核下，旧备用函数标记后续清理 |
...
- `AI Stock Radar` 页面标题改为“价格位置”。
- “返回 Radar 列表”改为“返回价格位置”。
- “AI 股票雷达研究”改为“价格位置研究”。
- 观察池副标题中的“AI 雷达观察池”改为“价格位置观察池”。
