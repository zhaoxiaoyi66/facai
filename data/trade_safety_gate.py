...
TRADE_SAFETY_FIELD_LABELS = {
    "actual_sell_pct": "实际卖出比例",
    "core_position_min_pct": "核心仓比例",
    "current_position_quantity": "当前持仓数量",
    "planned_sell_pct": "计划卖出比例",
    "quantity": "交易数量",
    "reentry_breakout_price": "突破回补价格",
    "reentry_buy_back_pct_on_breakout": "突破回补比例",
    "reentry_buy_back_pct_on_pullback": "回踩回补比例",
    "reentry_pullback_price": "回踩回补价格",
    "reentry_time_stop_days": "回补等待天数",
    "trading_position_max_pct": "交易仓比例",
    "unrealized_gain_pct": "浮动盈亏比例",
}
...
def _optional_non_negative_number(value: object, field: str) -> float | None:
    ...
    if number is None:
        return None
    if number < 0:
        raise ValueError(f"{_field_label(field)}不能为负数")
    return number
...
def _optional_number(value: object, field: str) -> float | None:
    ...
    try:
        ...
        raise ValueError(f"{_field_label(field)}需要填写数字") from exc
...
def _optional_int(value: object, field: str) -> int | None:
    ...
    try:
        ...
        raise ValueError(f"{_field_label(field)}需要填写整数") from exc
    if number < 0:
        raise ValueError(f"{_field_label(field)}不能为负数")
    return number
...
def _field_label(field: object) -> str:
    return TRADE_SAFETY_FIELD_LABELS.get(str(field or "").strip(), "该字段")
