...
def test_manual_review_missing_placeholders_are_localized() -> None:
    ...
    assert manual_review._review_display_text("待补", "") == ""
    assert manual_review._review_display_text("暂缺", "未记录") == "未记录"
    assert manual_review._review_display_text("none", "未记录") == "未记录"
    assert manual_review._review_has_display_text("N/A") is False
    assert manual_review._review_has_display_text("待补") is False
...
def test_manual_review_score_impact_copy_localizes_internal_status() -> None:
    ...
    assert manual_review._cn_item_type("NEW_INTERNAL_ITEM_TYPE") == "未归类"
    assert manual_review._cn_item_type("人工复核类型") == "人工复核类型"
    assert manual_review._cn_match("NEW_INTERNAL_MATCH_STATUS") == "不明确"
    assert manual_review._cn_match("人工匹配状态") == "人工匹配状态"
